...
class MFDiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], (H_patches, W_patches))
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    ...
    grid = np.meshgrid(grid_w, grid_h)  # W先
    grid = np.stack(grid, axis=0)
...
def quick_test():
    ...
    # 输入：2个样本，25通道，512x704
    x = torch.randn(2, 16, 512, 704)
    t = torch.randint(0, 1000, (2,))  # 随机时间步
    r = torch.randint(0, 1000, (2,))  # 随机扰动
    print(f" MFDiT 输入: {x.shape}, t: {t.shape}, r: {r.shape}")
    ...
    print(f"输出形状: {y.shape}")  # 期望 (2, 16, 512, 704)
    print("测试成功!,一次前向传播所用时间：",time.time()-start_time)
